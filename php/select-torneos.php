...
$fbardas="SELECT id FROM `creacion_torneo` WHERE `fecha`<=CURDATE() AND `fecha_limite`>=CURDATE() and disciplina='futbol bardas'";
$resultadoB = mysqli_query($conexion, $fbardas);
$mostrarB=mysqli_fetch_array($resultadoB);
$idBaas=$mostrarB['id'];

$fascenso="SELECT id FROM `creacion_torneo` WHERE `fecha`<=CURDATE() AND `fecha_limite`>=CURDATE() and disciplina='ascenso'";
$resultadoA = mysqli_query($conexion, $fascenso);
$mostrarA=mysqli_fetch_array($resultadoA);
$idAs = $mostrarA['id'];

$fifa="SELECT id FROM `creacion_torneo` WHERE `fecha`<=CURDATE() AND `fecha_limite`>=CURDATE() and disciplina='fifa'";
$resultadoF = mysqli_query($conexion, $fifa);
$mostrarF=mysqli_fetch_array($resultadoF);
$idFif = $mostrarF['id'];

$voley="SELECT id FROM `creacion_torneo` WHERE `fecha`<=CURDATE() AND `fecha_limite`>=CURDATE() and disciplina='voleibol'";
$resultadoV = mysqli_query($conexion, $voley);
$mostrarV=mysqli_fetch_array($resultadoV);
$idVol = $mostrarV['id'];

$basquet="SELECT id FROM `creacion_torneo` WHERE `fecha`<=CURDATE() AND `fecha_limite`>=CURDATE()and  disciplina='Basquetbol'";
